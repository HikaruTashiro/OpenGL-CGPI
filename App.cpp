...
void upperMenu(Image *&image, bool &imageLoaded, bool &greyfilter) {
  ...
  if (button_confirm &&
      (button_confirm = button_confirm && fileExists(filepath.c_str()))) {
    if (!image) {
...
void displayHistogram(Histogram histogram[], Image *&image, bool greyfilter) {
  ...
  if (image && (showHistogram = showHistogram && image)) {
    if (greyfilter)
      ImGui::PlotHistogram("Grey", plotHistogram, histogram[0].plot, RGB_AMOUNT,
                           0, NULL, FLT_MAX, FLT_MAX, ImVec2(0, 255));
    else {
      ImGui::PlotHistogram("RED", plotHistogram, histogram[0].plot, RGB_AMOUNT,
                           0, NULL, FLT_MAX, FLT_MAX, ImVec2(0, 255));
      ImGui::PlotHistogram("GREEN", plotHistogram, histogram[1].plot,
                           RGB_AMOUNT, 0, NULL, FLT_MAX, FLT_MAX,
                           ImVec2(0, 255));
      ImGui::PlotHistogram("BLUE", plotHistogram, histogram[2].plot, RGB_AMOUNT,
                           0, NULL, FLT_MAX, FLT_MAX, ImVec2(0, 255));
    }
  }
  ImGui::End();
