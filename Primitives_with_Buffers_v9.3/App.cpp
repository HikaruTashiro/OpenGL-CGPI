...
int main(void) {
  ...
  Shader *program_shader =
      new Shader("GLSL/vertShader.glsl", "GLSL/fragShader.glsl");
  ...
  while (!glfwWindowShouldClose(window)) {
      ...
      /*Change in state machine*/
      {
            ...
            mode) // if there are changes in states the following change as well
        {
          ...
          clearTemp(List_temporary,
                    count < 3); // if count < 3 is true then delete else dont
                                // (since it was added to the List_Primitives)
          select_window = false;
          count = 0;
        }
      }
    ...
    if (select_window && node_selected != nullptr) {
      ImGui::Begin("Select Primitives Window", NULL,
                   ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove);
      ImGui::TextColored(ImVec4(0.3411f, 0.7372f, 1.0f, 1.0f),
...
inline int buttonsImGui(int mode) {
  ...
  if (ImGui::Button("Select"))
    mode = SELECT;
  // ImGui::SliderFloat2("Transformation Slider (x, y)", tempTrans, 0.0f,
  // 960.0f);
  ImGui::SliderFloat("Transformation X: ", &tempXTrans,
...
void serializeButton(LList<Primitive *> &Lprimitives, const char *fileName) {
  if (ImGui::Button("Serialize to Json") && fileName[0] != '\0') {
    ...
    ordered_json obj;
    if (!file) {
      ImGui::TextColored(ImVec4(1.0f, 0.25f, 0.0f, 1.0f),
                         "Failed to Open File");
      return;
    }
    ...
    while (node_aux != nullptr) {
      ...
      case LINE:
        ...
        break;
      case RECTANGLE:
        obj["figura"]["retangulo"]
            << *dynamic_cast<Rectangle *>(node_aux->value);
        break;
      case TRIANGLE:
        obj["figura"]["triangulo"]
            << *dynamic_cast<Triangle *>(node_aux->value);
        break;
      case PLINE:
        obj["figura"]["linha poligonal"]
            << *dynamic_cast<PolygonLine *>(node_aux->value);
        break;
      case POLYGON:
